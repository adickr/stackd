...
/**
 * Response from GET /v1/journal/latest
 * - legacy relay returns ciphertext directly
 * - Irys relay returns pointer+metadata only
 */
...
function pickTimestampFromJson(x: any): number | undefined {
  return toNum(x?.createdAt) ?? toNum(x?.publishedAt);
}
...
// -------- Irys fetch --------
async function fetchIrysEncryptedBlob(pointer: string): Promise<{ nonceB64: string; ciphertextB64: string }> {
  const txId = pointer.startsWith("irys:") ? pointer.slice(5) : pointer;
  const url = `https://gateway.irys.xyz/${txId}`;

  const r = await fetch(url);
  if (!r.ok) throw new Error(`Irys fetch failed (${r.status})`);

  const j = await r.json();
  const nonceB64 = toStr(j?.nonceB64);
  const ciphertextB64 = toStr(j?.ciphertextB64);

  if (!nonceB64 || !ciphertextB64) {
    throw new Error("Irys blob malformed.");
  }
...
function assertHasCiphertext(
  x: CloudLatestResponse
): asserts x is CloudLatestResponse & { nonceB64: string; ciphertextB64: string } {
  if (!x.nonceB64 || !x.ciphertextB64) {
    throw new Error("Latest missing ciphertext.");
  }
}
...
class RelayCloudStorage implements CloudStorage {
  async challenge(walletAddress: string, snapshotHash: string) {
    ...
    const r = await fetch(
      `${base}/v1/journal/challenge?walletAddress=${encodeURIComponent(walletAddress)}&snapshotHash=${encodeURIComponent(snapshotHash)}`
    );
  ...
  async publish(req: CloudPublishRequest) {
    const base = mustBaseUrl();
    const r = await fetch(`${base}/v1/journal/publish`, {
      method: "POST",
    ...
    if (!r.ok) {
      ...
      throw mapFetchError(r.status, txt);
    }
  ...
  async latest(walletAddress: string) {
    const base = mustBaseUrl();
    const r = await fetch(`${base}/v1/journal/latest?walletAddress=${encodeURIComponent(walletAddress)}`);
    ...
    if (!out.pointer || !out.snapshotHash || !out.schemaVersion) {
      throw new Error("Latest malformed.");
    }

    if ((!out.nonceB64 || !out.ciphertextB64) && out.pointer.startsWith("irys:")) {
      const blob = await fetchIrysEncryptedBlob(out.pointer);
      out.nonceB64 = blob.nonceB64;
      out.ciphertextB64 = blob.ciphertextB64;
    }

    assertHasCiphertext(out);
    return out;
  }

  async creditsBalance(walletAddress: string) {
    const base = mustBaseUrl();
    const r = await fetch(`${base}/v1/credits/balance?walletAddress=${encodeURIComponent(walletAddress)}`);
    if (!r.ok) throw new Error(`Credits failed (${r.status})`);
    const raw = await r.json();
    return { walletAddress: toStr(raw?.walletAddress), credits: Number(raw?.credits ?? 0) };
  }
}
...
export function walletSigB64ToB58(sigB64: string) {
  ...
  if (bytes.length !== 64) throw new Error(`Wallet signature bytes length unexpected: ${bytes.length}`);
  return bs58.encode(bytes);
}

export function getCloudSigner() {
  if (!cloudSignMessages) throw new Error("Cloud signer not set");
