...
export default function JournalScreen() {
  ...
  const upsertCoin = useCoinStore((s) => s.upsertCoin);
  const clearAllStack = useStackStore((s) => s.clearAll);
  ...
  const handleRestore = () => {
    ...
    Alert.alert(
      "Restore inventory?",
      "This will replace your current stack entries with the ones from your latest seal. (Your coins list will be merged/updated.)",
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () => {
            try {
              // 1) Merge coins (minimal fields available in backup)
              for (const c of latestBackup.coins ?? []) {
                upsertCoin({
                  id: c.id,
                  createdAt: Date.now(),
                  name: c.name,
              ...
              // 2) Replace stack entries (ids will be regenerated by addEntry)
              clearAllStack();
              for (const e of latestBackup.entries ?? []) {
                addEntry({
              ...
              Alert.alert("Restored", "Your stack was restored from the latest seal.");
            } catch (e: any) {
        ...
        {!isConnected ? (
          ...
          <>
            ...
            <View style={styles.restoreCard}>
              ...
              <Text style={styles.restoreTiny}>
                Note: entry IDs are regenerated on restore (fine for now).
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  restoreTiny: { marginTop: 10, fontSize: 11, opacity: 0.55 },
